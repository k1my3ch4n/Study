...
- 자바스크립트는 싱글 스레드에서 작동 -> 한 번에 한 가지의 작업만 동기적으로 실행 , 즉 순서대로 실행 됨
- 이벤트 루프는 자바스크립트 런타임 외부에서 자바스크립트의 비동기 실행을 돕기 위해서 만들어진 장치
...
1. c 가 호출 스택에 들어감
2. c 내부 console.log 가 호출 스택에 들어감
3. console.log 실행
4. a 가 호출 스택에 들어감
5. a 내부 console.log 가 호출 스택에 들어감
6. console.log 실행
7. a 가 호출 스택에서 제거
8. b 가 호출 스택에 들어감
9. b 내부 console.log 가 호출 스택에 들어감
10. console.log 실행
11. b 가 호출 스택에서 제거
12. c 가 호출 스택에서 제거
13. 호출 스택이 비워짐
...
1. c 가 호출 스택에 들어감
2. console.log 가 호출 스택에 들어감
3. console.log 실행
4. `setTimeout(a(), 0)` 가 호출 스택에 들어감
5. 타이머 이벤트가 실행되면서 `태스트 큐` 에 들어가고, 스택에서 제거
6. b 가 호출 스택에 들어감
7. console.log 가 호출 스택에 들어감
8. console.log 실행
9. b 가 호출 스택에서 제거
10. c 가 호출 스택에서 제거
11. 이벤트 루프가 호출 스택이 비워진 걸 확인 후 , 태스트 큐에 있는 `setTimeout(a(), 0)` 를 호출 스택에 넣음
12. a 내부의 console.log 가 호출 스택에 들어감
13. console.log 실행
14. a 가 호출 스택에서 제거
15. 호출 스택이 비워짐

- 마이크로 태스크 큐도 존재하는데 , 이는 일반 태스크 큐 보다 우선순위가 높다. 여기에 예로는 promise 가 존재한다.
- 마이크로 태스크 큐가 실행된 후 렌더링 진행 , 그 후 태스크 큐 실행
- 각 마이크로 태스크 큐가 실행될 때 마다 렌더링의 기회를 얻게 됨
